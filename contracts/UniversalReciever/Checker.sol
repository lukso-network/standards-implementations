...
contract Checker {
    ...
    function lowLevelCheckImplementation(address target, bytes32 inter) external returns (bool) {
        (bool succ, bytes memory ret) = target.call(abi.encodeWithSignature("universalReciever(bytes32,bytes)", inter,""));
    ...
    function lowLevelCheckImplementationBytes(address target, bytes32 inter) external returns (bool) {
        (bool succ, bytes memory ret) = target.call(abi.encodeWithSignature("universalRecieverBytes(bytes32,bytes)", inter,""));
